...
import { Reservation, Tool, User, Transaction, TransactionType } from '../../../types';
...
interface UseRentalFormOptions {
    users: User[];
    tools: Tool[];
    onAddRental: (rental: Reservation) => void;
...
export const useRentalForm = (options: UseRentalFormOptions): UseRentalFormReturn => {
    ...
    const isUserBlocked = useMemo(() => {
        ...
        return new Date(selectedUser.membership_expiry) < new Date();
    }, [selectedUser]);
    ...
    const estimatedPrice = useMemo(() => {
        ...
        return weeks * selectedTool.daily_price;
    ...
    const handleSubmit = useCallback((e: React.FormEvent) => {
        ...
        if (selectedUser) {
            onUpdateUser({ ...selectedUser, total_debt: selectedUser.total_debt + finalPrice });
