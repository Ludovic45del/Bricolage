...
interface UseRentalFiltersReturn {
    ...
    // Helper
    getRentalWithDetails: (rental: Rental) => RentalWithDetails;
}
...
export const useRentalFilters = (options: UseRentalFiltersOptions): UseRentalFiltersReturn => {
    ...
    const getRentalWithDetails = (rental: Rental): RentalWithDetails => {
        const user = users.find(u => u.id === rental.userId);
        const tool = tools.find(t => t.id === rental.toolId);
        const isLate = rental.status === 'active' && parseISO(rental.endDate) < new Date();
